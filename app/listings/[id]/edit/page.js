...
export default function EditListingPage() {
  ...
  useEffect(() => {
    if (status === "authenticated" && session?.user?.role === "owner" && id) {
      fetchListing();
    } else if (status === "unauthenticated" || (status === "authenticated" && session?.user?.role !== "owner")) {
      setLoading(false);
    }
  }, [status, session, id]);
  ...
  const fetchListing = async () => {
    try {
      ...
      // Ownership check client side
      if (listing.owner._id !== session.user.id && listing.owner !== session.user.id) {
      ...
      setForm({
        ...
        roomType: listing.roomType || "single-room",
        tenantType: listing.tenantType || "any",
        street: listing.location?.street || "",
        area: listing.location?.area || "",
        thana: listing.location?.thana || "",
        googleMapsLink: listing.googleMapsLink || "",
        rent: listing.rent || "",
        advanceDeposit: listing.advanceDeposit || "",
        gasType: listing.gasType || "line",
        hasGeneratorOrIPS: listing.hasGeneratorOrIPS || false,
        hasWaterSupply247: listing.hasWaterSupply247 || false,
        hasCCTV: listing.hasCCTV || false,
        hasDarwan: listing.hasDarwan || false,
        gateLockTime: listing.gateLockTime || "",
        mealIncluded: listing.mealIncluded || false,
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4" style={{ background: "var(--background)" }}>
        <div className="text-center">
          ...
          <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>You need to be logged in to edit listings.</p>
          <Link href={`/login?callbackUrl=/listings/${id}/edit`} className="btn-primary inline-flex">Sign In</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4" style={{ background: "var(--background)" }}>
        <div className="text-center">
          ...
          <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>Only property owners can edit listings.</p>
